...
export function CardProduct({ data }: ICardProductProps) {
        ...
        <CardContent className="card__content">
          ...
          <Stack direction="row" alignItems="center">
            <Rating
              ...
              readOnly
            />
            <span style={{ color: "#787878", fontSize: "11px" }}>
              | Đã bán {data.sold}
            </span>
          </Stack>
          <Stack
          ...
          >
            <Typography
              ...
              color={`${data.discount !== 0 ? "#ff0000" : "#000000"}`}
              variant="h5"
              component="div"
            >
              {/* {data?.discount !== 0 ? (
            <>
              {numWithCommas(
                Math.round(data?.price * (1 - 0.01 * data.discount))
              )}{" "}
              ₫ <Box className="card__sale">{data?.discount}%</Box>
            </>
          ) : (
