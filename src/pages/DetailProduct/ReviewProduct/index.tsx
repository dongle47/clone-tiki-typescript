...
export interface IReviewProductProps {
  product: Product | null;
}

export default function ReviewProduct(props: IReviewProductProps) {
  ...
  if (count !== 0)
    avgRating = (
      reviews.reduce(
        (total, currentValue: any) => total + currentValue.rating,
        0
      ) / count
    ).toFixed(1);
  ...
  useEffect(() => {
    const getMyReviews = async () => {
      await reviewsApi
        .findReviews({ productId: props.product?.id })
        .then((res) => {
          setReviews(res);
        })
